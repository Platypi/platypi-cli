...
export default class BaseGenerator extends Generator {
    ...
    defineOptions(): void {
        ...
        this.option('dir', {
            aliases: ['d'],
            description: `Specify the path relative to \`app/src/${pluralize(
                this.type.toLowerCase()
            )}\` in which to store the ${this.type}`,
            defaults: '',
        });
        ...
        if (this.allowExtends || this.allowExtendsWithoutBase) {
            this.option('extends', {
                aliases: ['x'],
                description: `Specify the relative import path to the ${
                    this.type
                } to extend`,
            });
        }
        ...
        if (!this.utils.isString(options.name) && this.commands.length > 1) {
            ...
            promise = this.ui
                .prompt([
                    {
                        name: 'name',
                        type: this.ui.PROMPTS.CONFIRM,
                        default: true,
                        message: `Did you mean \`${this.buildFullCommand()
                            .slice(0, -1)
                            .join(' ')} -n ${name}\`?`,
                    },
                ])
    ...
    run(): any {
        ...
        if (ext === false && this.allowExtends) {
            ...
            config.ext = this.findExtends(dest)
                .replace(/\\/g, '/')
                .replace(/'/g, '');
        }
        ...
        if (this.allowExtends) {
            ...
            config.type = path
                .relative(path.resolve(root), path.resolve(root, dest, '..'))
                .replace(/\\|\//g, '-');
        }

        return this._render(src, dest, config).then(() => {
            return Promise.all([this.processMainTs(), this.processMainLess()]);
        });
    }

    protected promptName(name: string): Promise<string> {
    ...
    protected _render(src: string, dest: string, config: any): Promise<any> {
        let promises = [this.render(`${src}.ts`, `${dest}.ts`, config)];
    ...
    protected processMainTs(): Promise<any> {
    ...
    protected processMainLess(): Promise<any> {
        let destRoot = path.resolve(this.destRoot(), '..'),
            root = path.relative(this.project.root, destRoot),
            paths = [
    ...
    protected glob(
        ...
        return new Promise<Array<string>>((resolve, reject) => {
            ...
            if (files.length > 0) {
                return this.glob(root, files, ignore, replace).then(
                    (result) => {
    ...
    protected findExtends(root: string): string {
        ...
        return path.relative(
            path.resolve(dest, root, '..'),
            path.resolve(dest, `base/base.${this.ext}`)
        );
    }
    ...
    private formatMain(data: string, append: Array<string>): string {
        ...
        let sorted = this.sortMain(data.split(eol).concat(append))
            .map((value, index, lines) => {
                if (value.indexOf('/app/') > -1 && !appFound) {
                    appFound = true;
                    value = eol + value;
                } else if (
                    value.indexOf('/attributecontrols') > -1 &&
                    !acFound
                ) {
                    acFound = true;
                    value = eol + value;
                    ...
                    value.indexOf('/templatecontrols') > -1 &&
                    !tcFound
                ) {
                    tcFound = true;
                    value = eol + value;
        ...
        if (
            sorted[sorted.length - 2].trim() === '' &&
            sorted[sorted.length - 1].trim() === ''
        ) {
            sorted.pop();
        }
