...
export default class Generator extends Command {
    ...
    private _finishRender: Promise<any> = Promise.resolve();
    ...
    protected render(
        ...
        return (this._finishRender = this._finishRender
            .then(() => {
            ...
            .then((data: string) => {
                ...
                return this.file.read(dest, options).then(
                    () => {
                        this.ui.warn(
                            `The file \`${path.normalize(
                                dest.replace(this.project.root, '')
                            )}\` already exists.`
                        );
                        return this.ui
                            .prompt([
